...
import (
	...
	"github.com/moontrade/server/app"
...
func (db *database) del(key string) (prev *object, deleted bool) {
	//k := memory.WrapString(key)
	//v := db.keys.Delete(k.Pointer, k.Len())
...
func cmdSET(m app.Machine, args []string) (interface{}, error) {
	db := m.Data().(*database)
	if len(args) < 3 {
		return nil, app.ErrWrongNumArgs
	}
	var ex float64
...
func cmdSETRANDQUOTE(m app.Machine, args []string) (interface{}, error) {
	...
	return app.FilterArgs(args), nil
}
...
// DBSIZE
func cmdDBSIZE(m app.Machine, args []string) (interface{}, error) {
	db := m.Data().(*database)
	if len(args) != 1 {
		return nil, app.ErrWrongNumArgs
	}
...
// GET key
func cmdGET(m app.Machine, args []string) (interface{}, error) {
	db := m.Data().(*database)
	if len(args) != 2 {
		return nil, app.ErrWrongNumArgs
	}
...
func cmdKEYS(m app.Machine, args []string) (interface{}, error) {
	db := m.Data().(*database)
	if len(args) != 2 {
		return nil, app.ErrWrongNumArgs
	}
...
func cmdDEL(m app.Machine, args []string) (interface{}, error) {
	db := m.Data().(*database)
	if len(args) < 2 {
		return nil, app.ErrWrongNumArgs
	}
...
func multiPOWER(s app.Service) (interface{}, error) {
...
func cmdMONITOR(m app.Machine, args []string) (interface{}, error) {
	// Here we'll return a Hijack type that is just a function that will
	// take over the client connection in an isolated context.
	return app.Hijack(hijackedMONITOR), nil
}

func hijackedMONITOR(s app.Service, conn app.HijackedConn) {
	obs := s.Monitor().NewObserver()
	logger.Print("hijack opened: %s", conn.RemoteAddr())
	defer func() {
		logger.Print("hijack closed: %s", conn.RemoteAddr())
		obs.Stop()
		conn.Close()
	}()
...
func snapshot(data interface{}) (app.Snapshot, error) {
	db := data.(*database)
	snap := new(dbSnapshot)
