...
int art_tree_init(art_tree *t) {
    ...
    t->root = NULL;
    t->size = 0;
    t->bytes = sizeof(art_tree);
    return 0;
}

void art_tree_init_lock(art_tree *t) {
    if (!t->lock) {
//        t->lock = (moontrade_rwlock_t*)(void*)&t->_lock;
        t->lock = (void*)&t->_lock;
//        t->lock = (void*)new RWSpinLock();
    }
}
...
int art_tree_destroy(art_tree *t) {
    if (t->lock) {
        moontrade_rwlock_lock(t->lock);
        destroy_node(t->root);
        moontrade_rwlock_unlock(t->lock);
//        moontrade_rwlock_destroy(t->lock);
    } else {
        destroy_node(t->root);
    }
    return 0;
 ...
 */
art_value art_search(const art_tree *t, const unsigned char *key, int key_len) {
    if (t->lock) {
        moontrade_rwlock_lock_shared(t->lock);
    }
    art_node **child;
    art_node *n = t->root;
    ...
    while (n) {
        ...
        if (IS_LEAF(n)) {
            ...
            if (!leaf_matches((art_leaf *) n, key, key_len, depth)) {
                if (t->lock) {
                    moontrade_rwlock_unlock_shared(t->lock);
                }
...
static void add_child48(art_tree *t, art_node48 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 48) {
        ...
        *ref = (art_node *) new_node;
        free(n);
        t->bytes += sizeof(art_node256) - sizeof(art_node48);
...
static void add_child16(art_tree *t, art_node16 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 16) {
        ...
        *ref = (art_node *) new_node;
        free(n);
        t->bytes += sizeof(art_node48) - sizeof(art_node16);
...
static void add_child4(art_tree *t, art_node4 *n, art_node **ref, unsigned char c, void *child) {
    if (n->n.num_children < 4) {
        ...
        *ref = (art_node *) new_node;
        free(n);
        t->bytes += sizeof(art_node16) - sizeof(art_node4);
...
art_value art_insert(art_tree *t, const unsigned char *key, int key_len, art_value value) {
    ...
    if (!old_val) {
        t->size++;
        t->bytes += (uint64_t) sizeof(art_leaf);
    }
    if (t->lock) {
        moontrade_rwlock_unlock(t->lock);
    }
...
static void remove_child256(art_tree *t, art_node256 *n, art_node **ref, unsigned char c) {
    ...
    if (n->n.num_children == 37) {
        ...
        *ref = (art_node *) new_node;
        ...
        free(n);
        t->bytes -= (uint64_t) sizeof(art_node256) - (uint64_t) sizeof(art_node48);
    }
}
...
static void remove_child48(art_tree *t, art_node48 *n, art_node **ref, unsigned char c) {
    ...
    if (n->n.num_children == 12) {
        ...
        *ref = (art_node *) new_node;
        ...
        free(n);
        t->bytes -= (uint64_t) sizeof(art_node48) - (uint64_t) sizeof(art_node16);
    }
}
...
static void remove_child16(art_tree *t, art_node16 *n, art_node **ref, art_node **l) {
    ...
    if (n->n.num_children == 3) {
        ...
        *ref = (art_node *) new_node;
        ...
        free(n);
        t->bytes -= (uint64_t) sizeof(art_node16) - (uint64_t) sizeof(art_node4);
    }
}
...
static void remove_child4(art_tree *t, art_node4 *n, art_node **ref, art_node **l) {
    ...
    if (n->n.num_children == 1) {
        ...
        *ref = child;
        free(n);
        t->bytes -= (uint64_t) sizeof(art_node4);
    }
}
...
art_value art_delete(art_tree *t, const unsigned char *key, int key_len) {
    ...
    if (l) {
        t->size--;
        t->bytes -= (uint64_t) sizeof(art_leaf) + (uint64_t) l->key_len;
